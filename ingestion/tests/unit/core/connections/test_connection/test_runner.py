...
def test_failing_check_without_an_error_pack_does_not_crash():
    """The framework's guarantee is that a failure is a visible Failed result,
    never a crash - even if a provider omits its error pack."""
    ...
    result, _ = _run(Provider(), _Step("CheckAccess", Category.ConnectionGate))
    assert result.steps[0].status.value == "Failed"
    assert result.steps[0].errorLog == "boom"
