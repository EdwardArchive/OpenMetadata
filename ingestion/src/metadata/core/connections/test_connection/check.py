...
def collect_checks(provider: ChecksProvider) -> dict[str, CheckMethod]:
    ...
    Walks the class MRO so inherited checks are found and subclass overrides win,
    and reads only ``@check``-tagged methods - never instance data, so a provider
    property with side effects is never triggered.
    """
