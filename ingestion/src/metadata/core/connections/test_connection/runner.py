...
class TestConnectionRunner:
    ...
    def _failed_step(
        ...
        if self._step_timed_out(error, duration):
            ...
            # A provider without an error pack still fails visibly, never crashes.
            error_pack = getattr(self._checks, "errors", None)
            diagnosis = error_pack.classify(error) if error_pack else None
    ...
    def _patch(
        ...
        status = WorkflowStatus.Successful if final else WorkflowStatus.Running
        metadata.patch_automation_workflow_response(automation_workflow, result, status)
